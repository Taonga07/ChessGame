...
def on_click(event):
    ...
    try:
        if onclick == 1 and ((turn == 0 and board[row_number] [column_number].colour == 'white') or (turn == 1 and board[row_number] [column_number].colour == 'black')) or onclick == 2:
            currentText = square.cget('text')
            if onclick == 1:
                print('Where would you like to move your', board[row_number] [column_number].piece, 'to?')
                oldclour = board[row_number]
                [column_number].colour
