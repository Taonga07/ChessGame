...
class ChessGame():
    ...
    def check_against_check(self, clicked_piece):
        ...
        for row_number in range(0, 8):
            for column_number in range(0, 8):
                if self.board[row_number][column_number]!= None and self.board[row_number][column_number]!= clicked_piece.colour:
                    ...
                    for move in self.board[row_number][column_number].possible_moves:
                        ...
                        if (square != None) and (square.piece == 'King') and (square.colour == clicked_piece.colour): #our king is in check
                            ...
                            paths_to_king = paths_to_king + self.board[row_number][column_number].find_path_to_king(move[0], move[1])
    ...
    def on_click(self, event):
        ...
        if self.click == 0: # on fist click we are select a piece
            if (square_clicked != None)and(((self.turn == 0)and(square_clicked.colour == 'White'))or((self.turn == 1)and(square_clicked.colour == 'Black'))):
            ...
            if (current_square[0], current_square[1]) not in old_piece.possible_moves: # check possible move for piece
